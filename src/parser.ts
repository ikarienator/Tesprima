...
module TypedEsprima {
    ...
    export class Parser {
        ...
        inSwitch:bool = false;
        postProcess:(node:ASTNode)=> ASTNode = (node) => node;
        ...
        private createLiteral(token:Token) {
        ...
        parseBinaryExpression():Expression {
            ...
            var right:Expression = this.parseUnaryExpression(),
                prec:number;
            ...
            while ((prec = this.binaryPrecedence(lexer.lookahead, previousAllowIn)) > 0) {
                ...
                while ((suffixStack.length > 0) && (prec <= suffixStack[suffixStack.length - 1].precedence)) {
                    ...
                    if (lexer.includeLocation) {
                        right.loc = this.lexer.createLocationRange(item.lineNumber, item.columnNumber);
        ...
        parseConditionalExpression() {
            var expr:Expression, previousAllowIn:bool, consequent:Expression, alternate:Expression;
        ...
        parseAssignmentExpression():Expression {
            var token:Token, left:Expression, right:Expression;
            ...
            if (this.lexer.matchAssign()) {
                ...
                if (this.lexer.strict && left.type === Syntax.Identifier &&
                    this.lexer.isRestrictedWord((<Identifier>left).name)) {
        ...
        parseExpression():Expression {
            ...
            if (this.lexer.match(',')) {
                var seqExpr = this.factory.createSequenceExpression([ expr ]);
                ...
                while (this.lexer.index < this.lexer.length) {
                    ...
                    this.lexer.lex();
                    seqExpr.expressions.push(this.parseAssignmentExpression());
                }
                return seqExpr;
            }
            return expr;
        ...
        endMarker(startRange:number, startLine:number, startColumn:number, node:ASTNode) {
            ...
            if (this.lexer.includeLocation && typeof node.loc === 'undefined') {
                node.loc = this.lexer.createLocationRange(startLine, startColumn);
        ...
        wrapTrackingFunction(parseFunction:Function):Function {
            return () => {
                this.lexer.skipComment();
                var startRange = this.lexer.index,
                    startLine = this.lexer.lineNumber,
                    startColumn = this.lexer.index - this.lexer.lineStart;
                var node = <ASTNode> parseFunction.apply(this, arguments);
        ...
        patch() {
            ...
            if (this.lexer.includeRange || this.lexer.includeLocation) {
                ...
                Parser.PatchingMethods.forEach((method) => {
                    this[method] = this.wrapTrackingFunction(this[method]);
                });
            }
        }
