...
module TypedEsprima {
    ...
    export interface ILexer extends IErrorReporter {
        ...
        createLocationRange(startLine:number, startCol:number, endLine?:number, endCol?:number):SourceLocationRange;
    ...
    export class Lexer implements ILexer {
        ...
        public includeSource:string = null;
        ...
        public createLocationRange(startLine, startCol, endLine = this.lineNumber, endCol = this.index - this.lineStart):SourceLocationRange {
            var result:SourceLocationRange = {
                start: {
                    line: startLine,
                    column: startCol
                },
            ...
            if (this.includeSource) {
                result.source = this.includeSource;
            }
            return result;
        }
        ...
        // 7.4 Comments
        skipComment() {
            var ch:number, blockComment:bool, lineComment:bool;
        ...
        addComment(type:string, value:string, start:number, end:number, loc:SourceLocationRange) {
            assert(typeof start === 'number', 'CommentToken must have valid position');
            ...
            if (this.comments.length > 0) {
                if (this.lastCommentEnd > start) {
                    return;
                }
            } else {
                this.lastCommentEnd = 0;
            }
        ...
        scanComment() {
            var comment, ch:string, start, blockComment, lineComment;
                ...
                if (lineComment) {
                    ...
                    if (this.isLineTerminator(ch.charCodeAt(0))) {
                        ...
                        this.addComment('Line', comment, start, this.index - 1,
                            this.createLocationRange(startLine, startColumn, this.lineNumber, this.index - this.lineStart - 1   ));
                        ...
                        lineComment = false;
                        comment += ch;
                        this.addComment('Line', comment, start, this.length, this.createLocationRange(startLine, startColumn));
                    } else {
                        comment += ch;
                    }
                } else if (blockComment) {
                        ...
                        if (ch === '*') {
                            ...
                            if (ch === '/') {
                                ...
                                this.addComment('Block', comment, start, this.index,
                                    this.createLocationRange(startLine, startColumn));
                                comment = '';
                            }
                        }
                    }
                    ...
                    if (ch === '/') {
                        ...
                        if (this.index >= this.length) {
                            lineComment = false;
                            this.addComment('Line', comment, start, this.index, this.createLocationRange(startLine, startColumn));
                        }
                    } else if (ch === '*') {
                        start = this.index;
                        this.index += 2;
        ...
        collectToken() {
            ...
            if (token.type !== TokenType.EOF) {
                ...
                if (this.includeRange) {
                    tokenOutput.range = range;
                }
                if (this.includeLocation) {
                    tokenOutput.loc = this.createLocationRange(startLine, startColumn);
                }
                this.tokens.push(tokenOutput);
            }
        ...
        collectRegex() {
            ...
            var loc = this.createLocationRange(startLine, startColumn);
            ...
            if (!this.tokenize) {
                ...
                if (this.includeRange) {
                    tokenOutput.range = [pos, this.index];
                }
                if (this.includeLocation) {
                    tokenOutput.loc = loc;
                }
